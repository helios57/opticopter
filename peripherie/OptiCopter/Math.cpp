...
float Math::norm(int32_t *xyz) {
	return sqrt(square(xyz[0]) + square(xyz[1]) + square(xyz[2]));
}
void Math::normalizeVec(int32_t *xyz, float *out) {
	float n = norm(xyz);
	out[0] = xyz[0] / n;
	out[1] = xyz[1] / n;
	out[2] = xyz[2] / n;
}
float Math::norm(float *wxyz) {
	return sqrt(square(wxyz[0]) + square(wxyz[1]) + square(wxyz[2]) + square(wxyz[3]));
}
void Math::normalize(float *quat) {
	float n = norm(quat);
	quat[0] = quat[0] / n;
	quat[1] = quat[1] / n;
	quat[2] = quat[2] / n;
...
void Math::conjugate(const float *i_wxyz, float *o_wxyz) {
	o_wxyz[0] = i_wxyz[0];
	o_wxyz[1] = -i_wxyz[1];
	o_wxyz[2] = -i_wxyz[2];
	o_wxyz[3] = -i_wxyz[3];
...
void Math::multiply(const float *a_wxyz, const float *b_wxyz, float *o_wxyz) {
...
void Math::multiplyVec(const float *a_xyz, const float *b_wxyz, float *o_xyz) {
	float vec[] = { 0, a_xyz[0], a_xyz[1], a_xyz[2] };
	float quat[] = { b_wxyz[0], b_wxyz[1], b_wxyz[2], b_wxyz[3] };
 ...
 * @return
 */
void Math::fromEuler(const float pitch, const float yaw, const float roll, float *quat) {
