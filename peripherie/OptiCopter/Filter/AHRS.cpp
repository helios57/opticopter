...
void AHRS::getQuaternion(float* wxyz) {
	wxyz[0] = q0;
	wxyz[1] = q1;
	wxyz[2] = q2;
	wxyz[3] = q3;
...
void AHRS::getEuler(float* rollPitchYaw) {
	rollPitchYaw[0] = atan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1 * q1 + q2 * q2));
	rollPitchYaw[1] = asin(2 * (q0 * q2 - q1 * q3));
	//rollPitchYaw[2] = atan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q2 * q2 + q3 * q3));
}
