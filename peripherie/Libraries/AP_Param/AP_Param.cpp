                    ...
                    enum ap_var_type type, AP_HAL::BetterStream *port)
{
    switch (type) {
    case AP_PARAM_INT8:
        port->_printf_P((const prog_char *)PSTR("%s: %d\n"), s, (int)((AP_Int8 *)ap)->get());
        break;
    case AP_PARAM_INT16:
        port->_printf_P((const prog_char *)PSTR("%s: %d\n"), s, (int)((AP_Int16 *)ap)->get());
        break;
    case AP_PARAM_INT32:
        port->_printf_P((const prog_char *)PSTR("%s: %ld\n"), s, (long)((AP_Int32 *)ap)->get());
        break;
    case AP_PARAM_FLOAT:
        port->_printf_P((const prog_char *)PSTR("%s: %f\n"), s, ((AP_Float *)ap)->get());
        break;
    default:
        break;
    }
...
void AP_Param::convert_old_parameter(const struct ConversionInfo *info)
{
    ...
    if (ap2 == NULL) {
        hal.console->_printf_P((const prog_char *)PSTR("Unknown conversion '%S'\n"), info->new_name);
        return;
    }
    ...
    if (ptype == (ap_var_type)header.type) {
        ...
        hal.console->_printf_P((const prog_char *)(const prog_char *)PSTR("Bad conversion type '%S'\n"), info->new_name);
    }
}
