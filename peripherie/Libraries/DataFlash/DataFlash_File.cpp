...
{
    port->_printf_P((const prog_char *)PSTR("DataFlash: num_logs=%u\n"), 
                   (unsigned)get_num_logs());    
}

void DataFlash_File::ShowDeviceInfo(AP_HAL::BetterStream *port)
{
    port->_printf_P((const prog_char *)PSTR("DataFlash logs stored in %s\n"), 
                   _log_directory);
}
        ...
        if (filename != NULL) {
            ...
            if (size != 0) {
                ...
                if (stat(filename, &st) == 0) {
                    ...
                    port->_printf_P((const prog_char *)PSTR("Log %u in %s of size %u %u/%u/%u %u:%u\n"), 
                                   (unsigned)log_num, 
                                   filename,
                                   (unsigned)size,
