 ...
 */
void DataFlash_Class::_print_log_entry(uint8_t msg_type, uint8_t num_types, const struct LogStructure *structure, void (*print_mode)(AP_HAL::BetterStream *port, uint8_t mode), AP_HAL::BetterStream *port) {
	uint8_t i;
	...
	if (i == num_types) {
		port->_printf_P((const prog_char *) PSTR("UNKN, %u\n"), (unsigned) msg_type);
		return;
	}
	uint8_t msg_len = PGM_UINT8(&structure[i].msg_len) - 3;
	uint8_t pkt[msg_len];
	ReadBlock(pkt, msg_len);
	port->_printf_P((const prog_char *) PSTR("%S, "), structure[i].name);
	for (uint8_t ofs = 0, fmt_ofs = 0; ofs < msg_len; fmt_ofs++) {
		char fmt = PGM_UINT8(&structure[i].format[fmt_ofs]);
		switch (fmt) {
		case 'b': {
			port->_printf_P((const prog_char *) PSTR("%d"), (int) pkt[ofs]);
			ofs += 1;
			break;
		}
		case 'B': {
			port->_printf_P((const prog_char *) PSTR("%u"), (unsigned) pkt[ofs]);
			ofs += 1;
			break;
		}
		case 'h': {
			...
			port->_printf_P((const prog_char *) PSTR("%d"), (int) v);
			ofs += sizeof(v);
			break;
		}
		case 'H': {
			uint16_t v;
			memcpy(&v, &pkt[ofs], sizeof(v));
			port->_printf_P((const prog_char *) PSTR("%u"), (unsigned) v);
			ofs += sizeof(v);
			break;
		}
		case 'i': {
			int32_t v;
			memcpy(&v, &pkt[ofs], sizeof(v));
			port->_printf_P((const prog_char *) PSTR("%ld"), (long) v);
			ofs += sizeof(v);
			break;
		}
		case 'I': {
			uint32_t v;
			memcpy(&v, &pkt[ofs], sizeof(v));
			port->_printf_P((const prog_char *) PSTR("%lu"), (unsigned long) v);
			ofs += sizeof(v);
			break;
		}
		case 'f': {
			float v;
			memcpy(&v, &pkt[ofs], sizeof(v));
			port->_printf_P((const prog_char *) PSTR("%f"), v);
			ofs += sizeof(v);
			break;
		}
		case 'c': {
			int16_t v;
			memcpy(&v, &pkt[ofs], sizeof(v));
			port->_printf_P((const prog_char *) PSTR("%.2f"), 0.01f * v);
			ofs += sizeof(v);
			break;
		}
		case 'C': {
		...
		case 'n': {
			...
			memcpy(&v, &pkt[ofs], sizeof(v));
			v[sizeof(v) - 1] = 0;
			port->_printf_P((const prog_char *) PSTR("%s"), v);
			ofs += sizeof(v) - 1;
			break;
		}
		case 'N': {
		...
		if (ofs < msg_len) {
			port->_printf_P((const prog_char *) PSTR(", "));
		}
	}
	port->println();
}
 ...
 */
void DataFlash_Block::_print_log_formats(uint8_t num_types, const struct LogStructure *structure, AP_HAL::BetterStream *port) {
	for (uint8_t i = 0; i < num_types; i++) {
		const struct LogStructure *s = &structure[i];
		port->_printf_P((const prog_char *) PSTR("FMT, %u, %u, %S, %S, %S\n"), (unsigned) PGM_UINT8(&s->msg_type), (unsigned) PGM_UINT8(&s->msg_len), s->name, s->format, s->labels);
	}
}
 ...
 */
void DataFlash_Block::LogReadProcess(uint16_t log_num, uint16_t start_page, uint16_t end_page, uint8_t num_types, const struct LogStructure *structure, void (*print_mode)(AP_HAL::BetterStream *port, uint8_t mode), AP_HAL::BetterStream *port) {
	uint8_t log_step = 0;
...
void DataFlash_Block::DumpPageInfo(AP_HAL::BetterStream *port) {
	for (uint16_t count = 1; count <= df_NumPages; count++) {
		StartRead(count);
		port->_printf_P((const prog_char *) PSTR("DF page, log file #, log page: %u,\t"), (unsigned) count);
		port->_printf_P((const prog_char *) PSTR("%u,\t"), (unsigned) GetFileNumber());
		port->_printf_P((const prog_char *) PSTR("%u\n"), (unsigned) GetFilePage());
	}
}
...
void DataFlash_Block::ShowDeviceInfo(AP_HAL::BetterStream *port) {
	...
	ReadManufacturerID();
	port->_printf_P((const prog_char *) PSTR("Manufacturer: 0x%02x   Device: 0x%04x\n"), (unsigned) df_manufacturer, (unsigned) df_device);
	port->_printf_P((const prog_char *) PSTR("NumPages: %u  PageSize: %u\n"), (unsigned) df_NumPages + 1, (unsigned) df_PageSize);
}
...
void DataFlash_Block::ListAvailableLogs(AP_HAL::BetterStream *port) {
	...
	if (num_logs == 0) {
		port->_printf_P((const prog_char *) (const prog_char *) PSTR("\nNo logs\n\n"));
		return;
	}
	port->_printf_P((const prog_char *) (const prog_char *) PSTR("\n%u logs\n"), (unsigned) num_logs);
	...
	for (uint16_t i = num_logs; i >= 1; i--) {
		...
		port->_printf_P((const prog_char *) (const prog_char *) PSTR("Log %u,    start %u,   end %u\n"), (unsigned) temp, (unsigned) log_start, (unsigned) log_end);
...
/*
 write a parameter to the log
 */
void DataFlash_Class::Log_Write_Parameter(const AP_Param *ap, const AP_Param::ParamToken &token, enum ap_var_type type) {
char name[16];
...
for (ap = AP_Param::first(&token, &type); ap; ap = AP_Param::next_scalar(&token, &type)) {
