...
AP_InertialSensor::_init_gyro()
{
    ...
    hal.console->_printf_P((const prog_char *)PSTR("Init Gyro"));
...
AP_InertialSensor::_init_accel()
{
    ...
    hal.console->_printf_P((const prog_char *)PSTR("Init Accel"));
                                        ...
                                        float &trim_pitch)
{
    ...
    for (int8_t i=0; i<6; i++) {
        ...
        interact->_printf_P((const prog_char *)
        ...
        while (num_samples < 32) {
            if (!wait_for_sample(1000)) {
                interact->_printf_P((const prog_char *)PSTR("Failed to get INS sample\n"));
                return false;
            }
    ...
    interact->_printf_P((const prog_char *)PSTR("Offsets: %.2f %.2f %.2f\n"),
                       new_offsets.x, new_offsets.y, new_offsets.z);
    interact->_printf_P((const prog_char *)PSTR("Scaling: %.2f %.2f %.2f\n"),
                       new_scaling.x, new_scaling.y, new_scaling.z);

    if (success) {
        interact->_printf_P((const prog_char *)PSTR("Calibration successful\n"));
    ...
    interact->_printf_P((const prog_char *)PSTR("Calibration FAILED\n"));
