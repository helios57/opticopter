...
void AP_PerfMon::DisplayResults()
{
    ...
    hal.console->_printf_P((const prog_char *)PSTR("Fn:\t\tcpu\ttot(ms)\tavg(ms)\tmax(ms)\t#calls\tHz\n"));
    for( i=0; i<nextFuncNum; i++ ) {
        j=order[i];
        sumOfTime += time[j];
        ...
        hal.console->_printf_P((const prog_char *)PSTR("%-10s\t%4.2f\t%lu\t%4.3f\t%4.3f\t%lu\t%4.1f\n"),
            functionNames[j],
            pct,
    ...
    hal.console->_printf_P((const prog_char *)PSTR("unexpl:\t\t%4.2f\t%lu\n"),pct,(unsigned long)unExplainedTime/1000);
