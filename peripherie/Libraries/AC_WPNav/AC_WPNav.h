...
#include <APM_PI/APM_PI.h>             // PID library
...
public:
	...
	const Vector3f &get_loiter_target() const {
		return _target;
	}
	...
	const Vector3f &get_destination() const {
		return _destination;
	}
	...
	bool reached_destination() const {
		return _flags.reached_destination;
	}
	...
	int32_t get_desired_roll() const {
		return _desired_roll;
	}
	;
	int32_t get_desired_pitch() const {
	...
	float get_horizontal_velocity() {
		return _wp_speed_cms;
	}
	;

	/// get_climb_velocity - returns target climb speed in cm/s during missions
	float get_climb_velocity() const {
		return _wp_speed_up_cms;
	}
	;

	/// get_descent_velocity - returns target descent speed in cm/s during missions.  Note: always positive
	float get_descent_velocity() const {
		return _wp_speed_down_cms;
	}
	;

	/// get_waypoint_radius - access for waypoint radius in cm
	float get_waypoint_radius() const {
		return _wp_radius_cm;
	}

	/// get_waypoint_acceleration - returns acceleration in cm/s/s during missions
	float get_waypoint_acceleration() const {
		return _wp_accel_cms.get();
	}

	/// set_lean_angle_max - limits maximum lean angle
	void set_lean_angle_max(int16_t angle_cd) {
		if (angle_cd >= 1000 && angle_cd <= 8000) {
			_lean_angle_max_cd = angle_cd;
		}
	}
