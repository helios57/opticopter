...
static void register_write(uint8_t reg, uint8_t val) {
    hal.console->_printf_P((const prog_char *)PSTR("write reg %d val %d\r\n"),
            (int) reg, (int) val);
    spidev->cs_assert();
    spidev->transfer(reg);
    spidev->transfer(val);
...
static void setup() {
    hal.console->_printf_P((const prog_char *)PSTR("Initializing MPU6000\r\n"));
...
static void loop() {
    int16_t sensors[7];
    mpu6k_read(sensors);
    hal.console->_printf_P((const prog_char *)PSTR("mpu6k: %d %d %d %d %d %d %d\r\n"),
