...
void setup_pin(int pin_num) {
    hal.console->_printf_P((const prog_char *)PSTR("Setup pin %d\r\n"), pin_num);
...
void setup (void) {
    hal.console->_printf_P((const prog_char *)PSTR("Starting AP_HAL_AVR::Scheduler test\r\n"));
    ...
    hal.console->_printf_P((const prog_char *)PSTR("Testing delay callback. "
    ...
    hal.console->_printf_P((const prog_char *)PSTR("Testing failsafe callback. "
                "Pin %d should toggle at 1khz:\r\n"),
    ...
    hal.console->_printf_P((const prog_char *)PSTR("Testing running timer processes.\r\n"));
    hal.console->_printf_P((const prog_char *)PSTR("Pin %d should toggle at 1khz.\r\n"),
    ...
    hal.console->_printf_P((const prog_char *)PSTR("Test running a pathological timer process.\r\n"
                "Failsafe should continue even as pathological process "
                "dominates the processor."));
    hal.console->_printf_P((const prog_char *)PSTR("Pin %d should toggle then go high forever.\r\n"),
            (int) SCHEDULED_TOGGLE_PIN_2);
