...
void setup (void) {
    hal.console->_printf_P((const prog_char *)PSTR("RC input jitter test - Do not move sticks:\n"));
...
void loop (void) {
    ...
    if( update && hal.scheduler->millis() > time_trigger )
    {
        ...
        //hal.console->_printf_P((const prog_char *) PSTR("%c%c%c\r\n"), 0x1B, 0x5B, 0x48 ); // Terminal home position
        hal.console->_printf_P((const prog_char *) PSTR("\n< %.2u:%.2u:%.2u >------------------------------------- \n"), hour, minutte, second );
        for( uint8_t i = 0; i < 8; i++ )    
        {
            hal.console->_printf_P((const prog_char *) PSTR("ch%d: center:%d min:%d max:%d delta:%d"),
                                   i + 1, ppm_center[ i ], ppm_min[ i ],ppm_max[ i ], ppm_delta[ i ] );     

            if( ppm_mark[ i ] > 0 )
            {
                hal.console->_printf_P((const prog_char *) PSTR(" [%d]"), ppm_mark[ i ] );
                ppm_mark[ i ] = 0;
            }
