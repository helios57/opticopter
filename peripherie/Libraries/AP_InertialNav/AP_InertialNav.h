...
#include <AP_GPS/AP_GPS_Glitch.h>              // GPS Glitch detection library
...
public:
	...
	AP_InertialNav(const AP_AHRS* ahrs, AP_Baro* baro, GPS*& gps, GPS_Glitch& gps_glitch) :
			_ahrs(ahrs), _baro(baro), _gps(gps), _xy_enabled(false), _gps_last_update(0), _gps_last_time(0), _historic_xy_counter(0), _baro_last_update(0), _glitch_detector(gps_glitch), _error_count(0) {
	 ...
	 */
	void ignore_next_error() {
		_flags.ignore_error = 7;
	}
