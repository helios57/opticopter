...
const AP_Param::GroupInfo AP_Mount::var_info[] PROGMEM = {
    ...
    // @User: Standard
    AP_GROUPINFO("MODE", 0, AP_Mount, _mount_mode, MAV_MOUNT_MODE_RETRACT)
, // see MAV_MOUNT_MODE at ardupilotmega.h
...
have_roll = RC_Channel_aux::function_assigned(RC_Channel_aux::k_mount_roll) || RC_Channel_aux::function_assigned(RC_Channel_aux::k_mount2_roll);
have_tilt = RC_Channel_aux::function_assigned(RC_Channel_aux::k_mount_tilt) || RC_Channel_aux::function_assigned(RC_Channel_aux::k_mount2_tilt);
have_pan = RC_Channel_aux::function_assigned(RC_Channel_aux::k_mount_pan) || RC_Channel_aux::function_assigned(RC_Channel_aux::k_mount2_pan);
...
mavlink_msg_mount_status_send(MAVLINK_COMM_3, packet.target_system, packet.target_component, packet.pointing_a, packet.pointing_b, packet.pointing_c);
}
...
int32_t AP_Mount::angle_input(RC_Channel* rc, int16_t angle_min, int16_t angle_max) {
...
float AP_Mount::angle_input_rad(RC_Channel* rc, int16_t angle_min, int16_t angle_max) {
return radians(angle_input(rc, angle_min, angle_max) * 0.01f);
}

void AP_Mount::calc_GPS_target_angle(const struct Location *target) {
...
int16_t AP_Mount::closest_limit(int16_t angle, int16_t* angle_min, int16_t* angle_max) {
...
while (*angle_min < -1800)
	*angle_min += 3600;
while (*angle_min >= 1800)
	*angle_min -= 3600;
while (*angle_max < -1800)
	*angle_max += 3600;
while (*angle_max >= 1800)
	*angle_max -= 3600;
...
void AP_Mount::move_servo(uint8_t function_idx, int16_t angle, int16_t angle_min, int16_t angle_max) {
// saturate to the closest angle limit if outside of [min max] angle interval
