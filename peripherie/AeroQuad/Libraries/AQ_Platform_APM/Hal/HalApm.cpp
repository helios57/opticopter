...
#include "HalApm.h"
#include "AQ_Platform_APM/Interfaces/DigitalSource.h"
#include "AQ_Platform_APM/Interfaces/SPIDeviceDriver.h"
...
namespace arducopterNg {
	...
	HalApm::HalApm() {
		...
		mpu6k = new MPU6000(mpu6k_spi);
		mpu6k->init();

		ms5611 = new MS5611(ms5611_spi);
		ms5611->init();

		hmc5883l = new HMC5883L();
		hmc5883l->init();

		gps = new GPS_MTK(&Serial1, false);
		gps->init();
	}
