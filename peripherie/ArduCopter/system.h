...
static int8_t main_menu_help(uint8_t argc, const Menu::arg *argv) {
	hal.console->_printf_P((const prog_char *) PSTR("Commands:\n"
			"  logs\n"
			"  setup\n"
			"  test\n"
			"  reboot\n"
...
static void startup_ground(bool force_gyro_cal) {
	...
	// -----------------------------
	ins.init(force_gyro_cal ? AP_InertialSensor::COLD_START : AP_InertialSensor::WARM_START, ins_sample_rate);
#if CLI_ENABLED == ENABLED
	report_ins();
#endif
...
static uint32_t map_baudrate(int8_t rate, uint32_t default_baud) {
	...
	//hal.console->println_P(PSTR("Invalid SERIAL3_BAUD"));
	return default_baud;
}
...
static void print_flight_mode(AP_HAL::BetterStream *port, uint8_t mode) {
	...
	case SPORT:
		...
		break;
	default:
		port->_printf_P((const prog_char *) PSTR("Mode(%u)"), (unsigned) mode);
		break;
	}
}
