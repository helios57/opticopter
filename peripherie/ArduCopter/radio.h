...
static void init_rc_out()
{
    ...
    if(g.rc_3.control_in >= (g.throttle_max - 50)) {
        if(g.esc_calibrate == 0) {
            ...
            hal.console->_printf_P((const prog_char *)PSTR("Entering ESC Calibration: please restart APM.\n"));
            // turn on esc calibration notification
            AP_Notify::flags.esc_calibration = true;
            // block until we restart
            while(1) { delay(5); }
        }else{
            hal.console->_printf_P((const prog_char *)PSTR("ESC Calibration active: passing throttle through to ESCs.\n"));
            // clear esc flag
