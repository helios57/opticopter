...
print_log_menu(void)
{
    hal.console->_printf_P((const prog_char *)PSTR("logs enabled: "));

    if (0 == g.log_bitmask) {
        hal.console->_printf_P((const prog_char *)PSTR("none"));
    }else{
        if (g.log_bitmask & MASK_LOG_ATTITUDE_FAST) hal.console->_printf_P((const prog_char *)PSTR(" ATTITUDE_FAST"));
        if (g.log_bitmask & MASK_LOG_ATTITUDE_MED) hal.console->_printf_P((const prog_char *)PSTR(" ATTITUDE_MED"));
        if (g.log_bitmask & MASK_LOG_GPS) hal.console->_printf_P((const prog_char *)PSTR(" GPS"));
        if (g.log_bitmask & MASK_LOG_PM) hal.console->_printf_P((const prog_char *)PSTR(" PM"));
        if (g.log_bitmask & MASK_LOG_CTUN) hal.console->_printf_P((const prog_char *)PSTR(" CTUN"));
        if (g.log_bitmask & MASK_LOG_NTUN) hal.console->_printf_P((const prog_char *)PSTR(" NTUN"));
        if (g.log_bitmask & MASK_LOG_IMU) hal.console->_printf_P((const prog_char *)PSTR(" IMU"));
        if (g.log_bitmask & MASK_LOG_CMD) hal.console->_printf_P((const prog_char *)PSTR(" CMD"));
        if (g.log_bitmask & MASK_LOG_CURRENT) hal.console->_printf_P((const prog_char *)PSTR(" CURRENT"));
        if (g.log_bitmask & MASK_LOG_MOTORS) hal.console->_printf_P((const prog_char *)PSTR(" MOTORS"));
        if (g.log_bitmask & MASK_LOG_OPTFLOW) hal.console->_printf_P((const prog_char *)PSTR(" OPTFLOW"));
        if (g.log_bitmask & MASK_LOG_PID) hal.console->_printf_P((const prog_char *)PSTR(" PID"));
        if (g.log_bitmask & MASK_LOG_COMPASS) hal.console->_printf_P((const prog_char *)PSTR(" COMPASS"));
        if (g.log_bitmask & MASK_LOG_INAV) hal.console->_printf_P((const prog_char *)PSTR(" INAV"));
        if (g.log_bitmask & MASK_LOG_CAMERA) hal.console->_printf_P((const prog_char *)PSTR(" CAMERA"));
    }
...
dump_log(uint8_t argc, const Menu::arg *argv)
{
    ...
    if (dump_log == -2) {
        ...
        hal.console->_printf_P((const prog_char *)PSTR("dumping all\n"));
        Log_Read(0, 1, 0);
        return(-1);
    } else if ((argc != 2) || ((uint16_t)dump_log <= (last_log_num - DataFlash.get_num_logs())) || (static_cast<uint16_t>(dump_log) > last_log_num)) {
        hal.console->_printf_P((const prog_char *)PSTR("bad log number\n"));
        return(-1);
    }
...
select_logs(uint8_t argc, const Menu::arg *argv)
{
    ...
    if (argc != 2) {
        hal.console->_printf_P((const prog_char *)PSTR("missing log type\n"));
        return(-1);
    }
...
{
 #ifdef AIRFRAME_NAME
    hal.console->_printf_P((const prog_char *)PSTR((AIRFRAME_NAME)));
 #endif
    ...
    hal.console->println_P(PSTR(HAL_BOARD_NAME));
